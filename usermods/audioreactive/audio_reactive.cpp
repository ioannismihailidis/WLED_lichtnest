...
class AudioReactive : public Usermod {
  ...
  private:
    ...
    // Defer FFT/I2S sampling until after WiFi/AP bring-up. Starting the FFT task
    // reader during setup() can starve core0 networking so the AP never appears.
  ...
  public:
    ...
    void setup() override
    {
...
#endif
      // AI: below section was generated by an AI
      // Do NOT start the FFT task here. Creating/running it during setup() can prevent
      // WiFi softAP from coming up (no UI). loop() starts it after a short delay.
#ifdef ARDUINO_ARCH_ESP32
      const bool sourceOk = audioSource && audioSource->isInitialized();
      if (!sourceOk) {  // Still stay "enabled" — UDP Sound Sync may provide input
#ifdef WLED_DEBUG
      ...
      disableSoundProcessing = true;
      if (enabled && (sourceOk || dmType == SR_DMTYPE_NETWORK_ONLY)) deferAudioStart = true;
#else
      if (enabled) onUpdateBegin(false);                 // create FFT task (8266 / non-ESP32)
    ...
    void loop() override
    {
      ...
      // AI: below section was generated by an AI
      // Start FFT task only after WiFi/AP have had time to come up.
      if (deferAudioStart && (millis() > 4000) && !updateIsRunning) {
        deferAudioStart = false;
        if (enabled
#ifdef ARDUINO_ARCH_ESP32
            && (dmType == SR_DMTYPE_NETWORK_ONLY || (audioSource && audioSource->isInitialized()))
#endif
           ) {
          onUpdateBegin(false); // creates/resumes FFT task and clears disableSoundProcessing
#ifdef ARDUINO_ARCH_ESP32
          if (audioSource && FFT_Task == nullptr) {
            enabled = false;
            disableSoundProcessing = true;
          }
#endif
