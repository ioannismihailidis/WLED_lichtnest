  ...
  public:
    ...
    virtual void getSamples(FFTsampleType *buffer, uint16_t num_samples) {
      if (_initialized) {
        ...
        // Prefer blocking read (FFT starts only after WiFi/AP is up). Fall back: accept a
        // short timeout rather than dropping every batch — partial/timeout reads were
        // leaving the UI stuck at lvl=0 ("I2S digital - quiet").
