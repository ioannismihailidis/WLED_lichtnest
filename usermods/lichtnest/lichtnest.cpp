...
class Lichtnest : public Usermod {
    ...
    uint32_t computeColor(const FxParams& P, float elapsed, float x, float y, uint16_t chainIdx, uint16_t chainTotal, uint8_t tubeIdx, uint8_t tubeTotal, float along) {
      ...
      switch (P.fx) {
        case 0: { // Impuls — bands over 2D (linear/radial) or LED chain; optional travel easing via mode
          ...
          // Cap: 20 matches UI max; more bands = stack at rim when ease clamps (was hang trigger)
          uint8_t Nn = P.count < 1 ? 1 : (P.count > 20 ? 20 : P.count);
          float bestg = 2.0f;
          for (uint8_t k = 0; k < Nn; k++) {
            ...
            if (bounce) {
              ...
              // Off-field: without bounce, finished bands must leave — ease01 used to clamp
              // te≤1 so pos stuck at `travel` (rim pile-up → bright peak + hang on no-PSRAM).
  ...
  public:
    ...
    // AI: below section was generated by an AI
    // show() clears the frame then blends stock segments *before* this overlay runs.
    // Skipping without painting therefore reveals stock colours (amber tube / identify white).
    // Segment freeze does NOT help: frozen segments still blend their last buffer into the
    // cleared frame. On budget skip, black-fill geo pixels (cheap) instead of computeColor.
    void blackoutGeo () {
      for (uint8_t g = 0; g < geoCount; g++) {
    ...
    void handleOverlayDraw() override {
      ...
      if (_overlaySkipUntil && (int32_t)(frameStart - _overlaySkipUntil) < 0) {
        blackoutGeo();
        return;
      }
      refreshAudio();
  ...
  private:
    ...
    static void parseParams(JsonObject p, FxParams& P) {
      ...
      // Impuls: same field is travel easing — omitted mode must be Linear (0), not strobe default 1.
      if (P.fx == 0 && p["mode"].isNull()) P.mode = 0;
      ...
      if (P.fx == 0 && P.count > 20) P.count = 20;
