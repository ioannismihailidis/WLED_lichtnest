...
function ensureCoords ({ persist = false } = {}) {
  let added = false
  wled.segments.filter(isMappingTube).forEach((s, i) => {
...
const items = computed(() => wled.segments.filter(isMappingTube).map((s, i) => {
...
<template>
  <div>
    ...
    <p class="hint">
      ...
      <template v-else>Marker ziehen oder neu setzen. Genutzt als Ursprung bei Impuls · Radial.</template>
    </p>
